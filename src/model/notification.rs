...
impl Display for Notification {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        if self.status.to_uppercase().eq("CREATED") {
            write!(f, "Hello {}, let's try our new {} product: {}, only in BambangShop! Check it out: {}", 
            ...
            write!(f, "Hello {}, we informed that our {} product called {} already sold out...", 
                self.subscriber_name, self.product_type.to_lowercase(), self.product_title)
        } else {
            write!(f, "Hello {}, let's try our {} product: {}, grab it out before the stock ran out! Check it out: {}", 
                self.subscriber_name, self.product_type.to_lowercase(), self.product_title, self.product_url)
        }
    }
}